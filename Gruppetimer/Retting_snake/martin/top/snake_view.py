def get_color(value):
    """Return color based on the value in the grid."""
    if value == -1:
        return 'cyan'         # Apple
    elif value == 0:
        return 'lightgray'    # Empty cell
    else:
        return 'orange'       # Snake body

def draw_board(canvas, x1, y1, x2, y2, board, info_mode):
    rows = len(board) 
    cols = len(board[0])  
    
    # Beregn cellestørrelse
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            # Tegn cellen med riktig farge ved å bruke get_color-funksjonen
            canvas.create_rectangle(
                cell_left, cell_top, cell_right, cell_bottom, 
                fill=get_color(board[row][col])  # Bruker get_color her
            )

            # Tegn info-tekst i cellene hvis info_mode er aktivert
            if info_mode:
                text = f'{row},{col}\n{board[row][col]}'
                canvas.create_text(
                    (cell_left + cell_right) / 2,  
                    (cell_top + cell_bottom) / 2,  
