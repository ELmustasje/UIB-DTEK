...
def get_dog_facts(json_string):
    ...
    list = []
    first = dict['data'][0]['attributes']['body']
    second = dict['data'][1]['attributes']['body']
    last = dict['data'][2]['attributes']['body']
    list.append(first)
    list.append(second)
    list.append(last)
    return list 
...
def download_dog_facts():
    ...
    'User-Agent': 'dogapi.dog mathilde', # Noe som forteller hvem du er
    }

    webpage = requests.get(url, headers=headers)
    print(webpage.content) # Innholdet på nettsiden før dekoding: 'byte-streng'

    webpage_content = webpage.content.decode('utf-8').strip()
